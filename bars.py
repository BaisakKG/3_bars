...
def distance(lat1, lon1, lat2, lon2):
    ...
    distance2dot = 0.5 - cos((lat2-lat1)*degree)/2 + cos(lat1*degree)*cos(lat2*degree) * (1-cos((lon2-lon1)*degree)) / 2
    return 12742 * asin(sqrt(distance2dot))
...
def get_closest_bar(json_content, latitude, longitude):
    ...
    closest_bar = min(all_coordinates, key=lambda bar_coordinate: distance(human_coordinates['lat'], human_coordinates['lon'], bar_coordinate['lat'], bar_coordinate['lon']))
